...
# Shared shell helpers used across download, extraction, guest staging, and QEMU code.
...
# Stages the shared guest runtime and distro-specific postinst files on the FAT media.
guestlib_prep() {
    ...
    if postinst_file=$(retro_config_file postinst.sh); then
