...
qemu_config_find_file() {
    ...
    if [[ $# -eq 1 ]]; then
        ...
        log_error "Usage: qemu_config_find_file [DIR] FILE"
        return 1
    fi
...
qemu_display_apply_scaling() {
    ...
    backend=$(qemu_display_parse_backend "$QEMU_DISPLAY" || true)
    if [[ "$backend" != "cocoa" ]]; then
        return
    fi
    qemu_major=$(qemu_version_detect_major || true)
...
qemu_display_finish_config() {
    QEMU_DISPLAY="${QEMU_DISPLAY:-$(qemu_display_default_args)}"
    qemu_display_apply_scaling
    QEMU_ACCEL="${QEMU_ACCEL:--accel tcg}"
...
qemu_install_finish_config() {
    local install_script

    if [[ $COMMAND != "install" ]]; then
        return
    fi
    if [[ -z "${QEMU_INSTALL_SCRIPT:-}" ]] && install_script=$(qemu_config_find_file install.sh); then
...
qemu_config_load() {
    ...
    qemu_config_set_defaults
    if qemu_config=$(qemu_config_find_file qemu.sh); then
