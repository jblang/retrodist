...
`debian.sh` contains Debian 1.x base-install helpers. `deb091.sh` contains the
Debian 0.91 base-install and early `.deb` package install helpers. The public
wrappers in `common.sh` are:
...
`_debian_091_install_base` is the Debian 0.91 path. It extracts `basedsk1.img`
and `basedsk2.img` with `zcat | cpio`, rewrites root-device references in the
installed rc scripts and `lilo.conf`, runs `rdev`, installs LILO, and copies
`autoconf.sh` to `/sbin/setup.sh` in the target.
