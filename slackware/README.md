...
Automated Slackware 1.1.1 and later installs use host-generated tagfiles.
Slackware 1.01 and 1.0beta instead install every package in the fullest series
supported by their media through the original `doinstall` script. Three
conventional source tagsets are provided for tagfile-capable releases:
...
Select package series under `[install.packages]`. The shared dialog driver for
1.1.2 and later also accepts a staged tagfile path:
...
The 1.1.1 tty driver consumes tagfiles generated directly in its staged package
tree and does not accept `tagfile_path`.

Current staging reads the effective `full.tag`; a variant-level file replaces
one at the version level. Each line is `series package state`, where state is
`ADD`, `REC`, `OPT`, or `SKP`. Use `*` as the package to set the series default;
a package-specific rule overrides it. Run `retro tagfile CONFIG` to regenerate
`default.tag` from staged packages, then incorporate desired rules into
`full.tag` to affect automated staging.
...
choices. Behaviorally identical releases share range-named profiles, such as
`3.1-3.4` and `8.0-9.0`.
