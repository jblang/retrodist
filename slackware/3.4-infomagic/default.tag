...
a    bzip2        ADD   # bzip2 version 0.1pl2 (a block-sorting file compressor)
...
a    hdsetup      ADD   # The Slackware setup/package maintenance system v. 3.4.0
...
d    g77          OPT   # GNU Fortran-77 compiler, version 0.5.21.
d    gcc2723      ADD   # The GNU C compiler and support files for ELF (v. 2.7.2.3)
...
d    gxx2723      REC   # The GNU C++ compiler and support files for ELF (v. 2.7.2.3)
...
d    objc2723     OPT   # GNU compiler for the Objective-C language (v. 2.7.2.3)
...
d    perl1        OPT   # Larry Wall's interpreted systems language, part one. (v. 5.004_03)
d    perl2        OPT   # Larry Wall's interpreted systems language, part two. (v. 5.004_03)
d    perl3        OPT   # Larry Wall's interpreted systems language, part three. (v. 5.004_03)
...
n    *            SKP
n    apache       OPT   # Apache WWW server v 1.2.4
n    bind         REC   # BIND-8.1.1.
...
n    inn          OPT   # INN-1.5.1sec2 with innd-1.5.1
n    lynx         OPT   # Lynx 2.6
...
x    x3318514     REC   # An accelerated server for cards using IBM8514 chips.
x    x331agx      REC   # An accelerated server for IIT AGX chipsets.
x    x331bin      ADD   # Basic client binaries required for XFree86 3.3.1.
x    x331cfg      ADD   # Configuration files for XFree86 3.3.1.
x    x331doc      REC   # Documentation and release notes for XFree86 3.3.1.
x    x331f100     OPT   # 100-dpi screen fonts.
x    x331fcyr     OPT   # Cyrillic fonts for XFree86 3.3.1.
x    x331fno1     OPT   # Large fonts, part one.
x    x331fno2     OPT   # Large fonts, part two.
x    x331fnts     ADD   # Fonts for the X window system.
x    x331fscl     OPT   # Scaled fonts.
x    x331fsrv     OPT   # xfs (X font server)
x    x331html     OPT   # Docs for XFree86 in HTML format.
x    x331i128     REC   # A server for the Number Nine Imagine 128.
x    x331jdoc     OPT   # Japanese documentation and release notes for XFree86 3.3.1.
x    x331lib      ADD   # Dynamic libraries and configuration files for XFree86 3.3.1.
x    x331ma32     REC   # An accelerated server for cards using Mach32 chips.
x    x331ma64     REC   # An accelerated server for cards using the Mach64 chipset.
x    x331ma8      REC   # An accelerated server for cards using Mach8 chips.
x    x331man      REC   # Man pages for XFree86 3.3.1.
x    x331mono     REC   # A Monochrome server.
x    x331nest     OPT   # Xnest - a nested X server.
x    x331p9k      REC   # An accelerated server for cards using the P9000 chipset.
x    x331prog     REC   # Include files and configuration files for X programming.
x    x331prt      OPT   # Print-only server (Xprt) for XFree86 3.3.1.
x    x331ps       OPT   # XFree86 documentation in PostScript format.
x    x331s3       REC   # An accelerated server for cards using S3 chips.
x    x331s3v      REC   # An accelerated server for cards using S3 ViRGE chips.
x    x331set      OPT   # Graphical configuration utility for XFree86.
x    x331svga     REC   # A server for many SuperVGA video cards.
x    x331vfb      OPT   # Virtual framebuffer X server.
x    x331vg16     REC   # A server for 16 color EGA/VGA graphics modes.
x    x331w32      REC   # A server for chipsets in the ET4000/W32 series.
...
xd   *            SKP
xd   x331kit1     OPT   # XFree86 3.3.1 server linkkit, part one.
xd   x331kit2     OPT   # XFree86 3.3.1 server linkkit, part two.
xd   x331kit3     OPT   # XFree86 3.3.1 server linkkit, part three.
