...
a    gpm          REC   # General purpose mouse support v0.9
a    grep         ADD   # GNU grep 2.0
a    gzip         ADD   # GNU zip compression utilities. (v. 1.2.4)
a    hdsetup      ADD   # The Slackware setup/package maintenance system v. 2.1.0
a    idekern      REC   # Linux kernel version 1.1.59, without SCSI support.
a    keytbls      OPT   # kbd 0.88
a    ldso         ADD   # ld.so 1.4.4, the dynamic linker/loader.
a    less         ADD   # less-237
a    lilo         ADD   # LILO 0.15
a    loadlin      REC   # LOADLIN v1.5
a    lpr          ADD   # BSD print spooling system.
a    passwd       ADD
a    ps           ADD   # procps 0.97
a    scsikern     REC   # Linux kernel version 1.1.59, with SCSI + IDE support.
...
a    umsprogs     ADD   # umsdos_progs 0.3a
a    util         ADD   # util-linux 1.10
...
ap   shlbsvga     REC   # Shared library libsvga.so.1.1.8 (from svgalib 1.12)
ap   sudo         OPT   # sudo 1.2
ap   termbin      OPT   # Term 2.2.4
ap   termnet      OPT   # Network utilities for term
ap   termsrc      OPT   # Term 2.2.4 source code
...
d    flex         ADD   # flex - fast lexical analyzer generator version 2.4.7
d    gcc          REC   # The GNU C compiler and support files (v. 2.5.8)
d    gdb          REC   # The GNU debugger. (v. 4.13)
...
d    lx1159_1     REC   # Linux kernel source version 1.1.59, part one.
d    lx1159_2     REC   # Linux kernel source version 1.1.59, part two.
d    lx1159_3     REC   # Include files for Linux 1.1.59. (Kernel source 1.1.59, part three)
d    m4           REC   # GNU m4 1.3
...
d    svgalib      OPT   # Svgalib Super-VGA Graphics Library (v1.12)
d    tools        OPT   # tools 2.16

e    *            SKP
e    elisp1       OPT   # Part one of the Lisp source files for Emacs 19.27.
e    elisp2       OPT   # Part two of the Lisp source files for Emacs 19.27.
e    elispc       REC   # These are compiled lisp files for GNU Emacs 19.27.
e    emac_nox     OPT   # A replacement /usr/bin/emacs-19.27 binary that is not compiled with
e    emacmisc     REC   # Miscellaneous files for emacs 19.27.
e    emacsbin     ADD   # GNU Emacs 19.27
...
n    dip          OPT   # DIP - dialup IP connection handler 3.3.7i
...
n    sendmail     REC   # BSD sendmail 8.6.9.
...
q    *            SKP
q    alpha        OPT   # Linux kernel 1.1.59 with some alpha SCSI drivers.
q    cdu31a       OPT   # Linux kernel version 1.1.59, with SCSI+IDE+Sony CDU31/33A CD support.
q    cdu31ao      OPT   # Linux kernel version 1.1.18, with SCSI+IDE+Sony CDU31A CD support.
q    cdu535       OPT   # Linux kernel version 1.1.59, with SCSI+IDE+Sony CDU531/535 CD support.
q    idekerno     OPT   # Linux kernel version 1.1.18, without SCSI support.
q    mitsumi      OPT   # Linux kernel version 1.1.59, with SCSI+IDE+Mitsumi CD support.
q    nec260       OPT   # Linux kernel version 1.1.59, with SCSI+IDE+NEC260 IDE CD support.
q    sbpcd        OPT   # Linux kernel version 1.1.59, with SCSI+IDE+Soundblaster CD support.
q    sbpcdo       OPT   # Linux kernel version 1.1.18, with SCSI+IDE+SoundBlaster Pro CD
q    scsikrno     OPT   # Linux kernel version 1.1.18, with SCSI + IDE support.
q    sony535o     OPT   # Linux kernel version 1.1.18, with SCSI+IDE+Sony 535/531 CD support.
q    xt           OPT   # Linux kernel version 1.1.59, with IDE+XT (MFM/RLL) drive support.
...
x    x_agx        REC   # An accelerated server for IIT AGX-016, AGX-015, and AGX-014 chipsets.
...
x    x_p9000      REC   # An accelerated server for cards using the P9000 chipset.
x    x_s3         REC   # An accelerated server for cards using S3 chips.
x    x_svga       REC   # A server for SuperVGA video cards.
x    x_vga16      REC   # A server for 16 color EGA/VGA graphics modes.
x    x_w32        REC   # A server for chipsets in the ET4000/W32 series.
x    xextra       OPT   # Two optional experimental servers for X11R6.
x    xf_bin       ADD   # Basic client binaries required for XFree86 3.1.
x    xf_cfg       ADD   # XDM configuration, chooser, and FVWM 1.23b.
x    xf_ctrb      REC   # Selected contrib programs for X11R6:
x    xf_doc       REC   # Documentation and release notes for XFree86 3.1.
x    xf_lib       ADD   # Dynamic libraries and configuration files for XFree86 3.1.
x    xfnt         ADD   # Fonts for the X window system.
x    xfnt75       REC   # 75dpi Fonts for the X window system.
x    xinclude     REC   # Header files for X11 programming.
x    xlock        REC   # xlockmore-1.14
x    xman.tgz     REC
x    xpm          ADD   # The Xpm shared and static libraries, v. 3.4c (with libXpm.so.4.3)
x    xusrbin      OPT   # Rstartd daemon.
...
xd   *            SKP
xd   xf_kit1      OPT   # XFree86 3.1 Linkkit (part one)
xd   xf_kit2      OPT   # XFree86 3.1 Linkkit (part two)
xd   xf_pex       OPT   # XFree86 3.1 PEX distribution
