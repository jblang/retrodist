...
README for Slackware Linux 7.1.0.

Slackware 7.1 is a complete distribution of the Linux operating system.  This
version contains the 2.2.16 Linux kernel, plus recent versions of these (and
other) software packages:

- Kernel modules         2.2.16
- PPP daemon             2.3.11
- Dynamic linker (ld.so) 1.9.9
- C compiler             egcs-1.1.2, gcc-2.7.2.3, and gcc-2.95.2.
- Binutils               2.9.1.0.25
- C Library              glibc-2.1.3
- C++ Library            libstdc++-2-libc6.1-1-2.9.0
- Procps                 2.0.6
- Gpm                    1.19.2
- SysVinit               2.78
- Shadow Password Suite  shadow-19990827
- Util-linux             util-linux-2.10l
...
slakware/a1 - slakware/a16:  The base system.  Enough to get up and running and
                have elvis and comm programs available.  Based around the 
                2.2.16 Linux kernel, and concepts from the Linux filesystem
                standard.  Installing the entire A series requires 50 MB.
...
slakware/ap1:   Linux applications.  These are some useful programs, including
                ...
                other utilities.  Installing the entire AP series uses 36 MB.

slakware/d1:    Program development.  egcs-1.1.2 (gcc-2.8 based C/C++/f77/
                Objective-C compiler from egcs.cygnus.com), make (GNU and BSD),
                byacc and GNU bison, flex, glibc-2.1.3, gdb, SVGAlib, ncurses,
                gcl (LISP), p2c, m4, perl, python, rcs.  Installing the entire
                D series will require 180 MB.

slakware/des1:  glibc-crypt() add-on.  This is needed if you care about
                supporting old-style DES password encryption.  Without it,
                only MD5 password encryption will be available.  It's probably
                good to install, but isn't strictly required.  The crypt()
                add-on requires less than one megabyte of drive space.
...
slakware/e1:    GNU Emacs 20.7.  This is a text editor with about a million
                ...
                need to do.  Installing the entire E series will require 45 MB.

slakware/f1:    A collection of FAQs and other documentation.  Installing the
                F series requires about 14 MB.
...
slakware/gtk1:  Applications for X that use the GTK+ toolkit, and GNOME.  This
                ...
                environment for X.  Installing the GTK series requires 135 MB.

slakware/k1:    Source code for the 2.2.16 Linux kernel.  You'll need this
                (along with the C compiler and utilities from the D series) if
                you want to recompile your Linux kernel.  Installing the K
                series will require 72 MB, and you'll need more to compile it.
...
slakware/kde1:  The K Desktop Environment and the Qt libraries.  KDE is an
                ...
                software development.  Installing KDE requires 88 MB.

slakware/n1 - slakware/n8:  Networking.  This package contains TCP/IP and UUCP
...
slakware/t1:    teTeX is Thomas Esser's TeX distribution.  TeX is a typesetting
                ...
                series requires 125 MB.

slakware/tcl1:  Tcl, Tk, TclX, Tix, expect;  built with ELF shared libraries
                and dynamic loading support.  The TCL series needs about 17 MB.
...
slakware/x1:    The X Window System (XFree86 3.3.6) with libXpm, fvwm2-2.2.4,
                and xlock added.  Also includes xf86config and XF86Setup, two
                utilities to configure XFree86 automatically.  The entire X
                series requires 100 MB.

slakware/xap1:  Applications for the X Window System.  Extra programs for X,
                such as file managers (TkDesk, xfm), a window manager that
                makes X resemble Windows95 (fvwm95), Netscape Communicator,
                an image viewer (xv), a fractal generator (xfractint),
                communications programs, and more.  Installing the entire XAP
                series will require about 64 MB.
...
slakware/xv1:   xview3.2p1-X11R6.  The XView series adds support for the Open
                ...
                compiling XView applications.  The XV series uses 14 MB.
...
zipslack        This directory contains ZipSlack, a version of Slackware
                supplied as a single 36 megabyte ZIP archive.  Installation
...
bigslack        This is similar to ZipSlack, but includes a much more
                complete system using about 850 megabytes of drive space.
                Includes X, GNOME, networking apps, servers, development tools.
                The works.
...
Slackware's A (base) and most of the N (network) software sets can be
installed from floppy disks as well.  It's pretty rare these days for anyone
to attempt an installation from floppy disk, and it's usually only done on
laptops or other machines that have no CD-ROM drive or modem (i.e. there is
no other choice).  Note that the last N directory will be too large to fit
on a floppy disk, so you'll have to install those packages later.
...
contains special kernel modules to recognize PCMCIA devices.  With either of
these choices, you will require a supplemental hardware support disk in
...
This will take about 330 megabytes, so if you don't have that much space you'll
