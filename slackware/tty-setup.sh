...
tty_setup_mount_staging_partition() {
    ...
    script_send_line "$FAT_PARTITION"
    script_wait_line "Where would you like to mount $FAT_PARTITION?"
    script_send_line "$FAT_MOUNT"
...
tty_setup_configure_network() {
    ...
    script_wait_line "Enter domain name for $NET_HOSTNAME:"
    script_send_line "$NET_DOMAINNAME"
    script_wait_line "Do you plan to ONLY use loopback ([y]es, [n]o)?"
    script_send_line "n"
    script_wait_line "Enter IP address for $NET_HOSTNAME (aaa.bbb.ccc.ddd):"
    ...
    script_wait_line "Name Server for domain $NET_DOMAINNAME (aaa.bbb.ccc.ddd):"
...
tty_setup_run_first_boot_autoconf() {
    script_wait_line "$NET_HOSTNAME login:"
    script_send_line root
    script_wait_line "$NET_HOSTNAME:~#"
    script_send_line "$FAT_MOUNT/autoinst.d/autoconf.sh"
}
