...
x11_backup_orig() {
    if [ -f "$1" ]; then
        log DEBUG "Creating backup file: $1.orig"
        cp "$1" "$1.orig"
    fi
}
...
x11_detect_mouse_defaults() {
    ...
    if [ -z "$X11_MOUSETYPE" ]; then
        ...
        /dev/cua* | /dev/ttyS*)
            ...
            ;;
        *)
            log ERROR "Unknown mouse device: $X11_MOUSEDEV"
            exit 1
            ;;
        esac
    fi
    log INFO "X11 mouse configuration:"
    log INFO "  X11_MOUSEDEV=$X11_MOUSEDEV"
    log INFO "  X11_MOUSETYPE=$X11_MOUSETYPE"
}
...
x11_detect_path() {
    ...
    for X11_CANDIDATE in "$@"; do
        log DEBUG "Checking X11 config directory: $X11_CANDIDATE"
        if [ -d "$X11_CANDIDATE" ]; then
            X11_PATH="$X11_CANDIDATE"
            log INFO "Selected X11 config directory: $X11_PATH"
            return 0
        fi
    done
    return 1
...
x11_link_var_x11r6_bin() {
    if [ -d /var/X11R6/bin ]; then
        log INFO "Creating symlink: /var/X11R6/bin/X -> $X11_SERVER"
        (
            cd /var/X11R6/bin || exit 1
            ln -sf "$X11_SERVER" X
        )
    else
        log DEBUG "No /var/X11R6/bin directory; skipping X server symlink"
    fi
}
...
x11_install_startx_setfont_wrapper() {
    for X11_STARTX in /usr/bin/startx /usr/X386/bin/startx; do
        if [ -f "$X11_STARTX" ] && [ ! -L "$X11_STARTX" ] && [ ! -f "$X11_STARTX.real" ]; then
            log INFO "Creating backup file: $X11_STARTX.real"
            mv "$X11_STARTX" "$X11_STARTX.real"
            log INFO "Creating file: $X11_STARTX"
            x11_build_startx_setfont_wrapper >"$X11_STARTX"
            chmod 755 "$X11_STARTX"
        else
            log DEBUG "Skipping startx wrapper candidate: $X11_STARTX"
        fi
    done
}
...
x11_write_xf86config() {
    ...
    log INFO "Creating file: $X11_XFREECFG"
    cat >"$X11_XFREECFG"

    if [ "$X11_XFREECFG" != "$X11_XFREECFG_ETC" ]; then
        log INFO "Creating symlink: $X11_XFREECFG_ETC -> $X11_XFREECFG"
...
x11_3x4x_common_config() {
    ...
    log INFO "X11 configuration:"
    log INFO "  X11_DEPTHS=$X11_DEPTHS"
    log INFO "  X11_DEFAULT_DEPTH=$X11_DEFAULT_DEPTH"
    log INFO "  X11_MODES=$X11_MODES"
    log INFO "  X11_CHIPSET=$X11_CHIPSET"
    log INFO "Found X11 server at $X11_SERVER"
    x11_link_var_x11r6_bin

    x11_detect_path "$@" || return 1
    log INFO "XF86Config path is $X11_PATH/XF86Config"
}

# Configure XFree86 3.x using the SVGA server.
x11_3x_config() {
    X11_SERVER=/usr/X11R6/bin/XF86_SVGA
    log INFO "Using X11 configuration style: XFree86 3.x SVGA"
...
x11_4x_config() {
    ...
    log INFO "Using X11 configuration style: XFree86 4.x"
...
x11_1x2x_common_config() {
    log INFO "Found X11 server at $X11_SERVER"
    log INFO "Creating symlink: $(dirname "$X11_SERVER")/X -> $(basename "$X11_SERVER")"
    (
    ...
    log INFO "Xconfig path is $X11_XCONFIG"
...
x11_1x2x_svga_config() {
    ...
    X11_MODES=${X11_MODES:-'"1024x768" "800x600" "640x480"'}
    log INFO "Using X11 configuration style: XFree86 1.x/2.x SVGA"
    log INFO "X11 configuration:"
    log INFO "  X11_MODES=$X11_MODES"
    x11_1x2x_common_config || return 1
    log INFO "Creating file: $X11_XCONFIG"
...
x11_1x2x_mono_config() {
    ...
    log INFO "Using X11 configuration style: XFree86 1.x/2.x monochrome"
    log INFO "X11 configuration:"
    log INFO "  X11_MODES=$X11_MODES"
    x11_1x2x_common_config || return 1
    log INFO "Creating file: $X11_XCONFIG"
...
_x11_config() {
    log_div
    log INFO "Configuring X11..."

    x11_detect_mouse_defaults

    # Detect the variant of X11 based on the existence of server binaries.
    if [ -x /usr/X11R6/bin/XFree86 ]; then
        log INFO "Detected XFree86 4.x server"
        x11_4x_config
    elif [ -x /usr/X11R6/bin/XF86_SVGA ]; then
        log INFO "Detected XFree86 3.x SVGA server"
        x11_3x_config
    elif [ -x /usr/X386/bin/XF86_SVGA ]; then
        log INFO "Detected XFree86 1.x/2.x SVGA server"
        x11_1x2x_svga_config
    elif [ -x /usr/X386/bin/X386mono ]; then
        log INFO "Detected XFree86 1.x/2.x monochrome server"
        x11_1x2x_mono_config
    else
        log WARN "No supported X11 server found."
    fi
}
