...
tty_set_defaults() {
    ...
    ttyS[0-9]*)
        ...
        log DEBUG "Using alternate serial device spelling: $TTY_DEV_ALT"
        ;;
    ttys[0-9]*)
        TTY_DEV_ALT=ttyS${TTY_DEV#ttys}
        TTY_ID=${TTY_ID:-s${TTY_DEV#ttys}}
    ...
    *)
    ...
    log INFO "TTY configuration:"
    log INFO "  TTY_DEV=$TTY_DEV"
    log INFO "  TTY_DEV_ALT=$TTY_DEV_ALT"
    log INFO "  TTY_ID=$TTY_ID"
    log INFO "  TTY_BAUD=$TTY_BAUD"
    log INFO "  TTY_RUNLEVELS=$TTY_RUNLEVELS"
}

# Populate target paths for serial-console configuration files.
tty_detect_paths() {
    TTY_INITTAB="$ETC_D/inittab"
    TTY_INITTAB_NEW="$ETC_D/inittab.new"
    TTY_LOGIN_DEFS="$ETC_D/login.defs"
    TTY_LOGIN_DEFS_NEW="$ETC_D/login.defs.new"
    TTY_SECURETTY="$ETC_D/securetty"

    if [ ! -f "$TTY_INITTAB" ]; then
        log WARN "No inittab found at $TTY_INITTAB; skipping serial console configuration"
        return 1
    fi
    log DEBUG "TTY paths:"
    log DEBUG "  inittab=$TTY_INITTAB"
    log DEBUG "  login_defs=$TTY_LOGIN_DEFS"
    log DEBUG "  securetty=$TTY_SECURETTY"
}
...
tty_find_getty_line() {
    ...
    if [ -n "$TTY_STOCK_LINE" ]; then
        log DEBUG "Found serial getty line: $TTY_STOCK_LINE"
        # Determine which device to use based on what we want
        TTY_STOCK_DEV=$TTY_DEV
    else
        log DEBUG "No serial getty line found in inittab"
    fi
}
...
tty_write_inittab() {
    if [ -n "$TTY_STOCK_LINE" ]; then
        log INFO "Enabling serial getty entry for $TTY_STOCK_DEV"
        # Uncomment the line
        ...
        log DEBUG "Generated line: $TTY_NEW_LINE"
...
" "$TTY_INITTAB" >"$TTY_INITTAB_NEW"
        ...
        log WARN "No serial getty line found; leaving inittab unchanged"
        return 1
    fi
}

# Run one tty config step without letting its failure stop later postinst steps.
tty_run_step() {
    if "$1"; then
        :
    else
        log WARN "$1 failed for $TTY_DEV; skipping remaining tty configuration"
        return 1
    fi
}
...
tty_config_inittab() {
    if tty_find_getty_line; then
        ...
        log WARN "Active getty line already exists for $TTY_DEV; leaving inittab unchanged"
        return 0
    fi
...
tty_config_login_defs() {
    if [ -f "$TTY_LOGIN_DEFS" ]; then
        ...
        log INFO "Creating file: $TTY_LOGIN_DEFS"
        ...
        log DEBUG "No login.defs found at $TTY_LOGIN_DEFS"
    fi
}
...
tty_config_securetty() {
    ...
    if [ -f "$TTY_SECURETTY" ]; then
        if grep "^$TTY_DEV\$" "$TTY_SECURETTY" >/dev/null 2>&1; then
            log INFO "$TTY_DEV already present in $TTY_SECURETTY"
            return 0
        fi
    fi
    log INFO "Updating file: $TTY_SECURETTY"
...
_tty_config() {
    log_div
    tty_set_defaults
    log INFO "Configuring serial console on $TTY_DEV..."
